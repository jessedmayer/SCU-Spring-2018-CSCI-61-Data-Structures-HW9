...
public:
    ...
    size_t find_longest_dist(size_t start, size_t dest){
        int size = n();
        bool marked[size];
        for (int i = 0; i < n(); i++) {
            marked[i] = false;
        }
        list<size_t> n = neighbors(start);
        size_t max = 0;
        for(list<size_t>::iterator it = n.begin(); it!=n.end(); it++){
            bool new_marked[size];
            for (int i = 0; i < size; i++) {
                new_marked[i] = marked[i];
            }
            new_marked[*it] = true;
            size_t new_max = labels[start][*it];
            size_t current = find_longest_dist_rec(*it,dest,new_marked, new_max);
            if(current > max){
                max = current;
            }
        }
    ...
    size_t find_longest_dist_rec(size_t start, size_t dest, bool marked[], size_t max){
        list<size_t> n = neighbors(start);
